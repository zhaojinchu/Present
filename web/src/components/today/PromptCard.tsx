...
import { Badge, Button, Card, cx, Icon, Txt } from '@/ui';
import { CountdownRing } from './CountdownRing';
import { ExcuseSheet } from './ExcuseSheet';
...
export function PromptCard({
  ...
  const cantMakeIt = () => setExcusing(true);
  const excuseSheet = <ExcuseSheet occurrence={o} open={excusing} onOpenChange={setExcusing} />;
  ...
  const friendsLine =
    friendsPosted.length > 0 ? (
      <div className="flex items-center gap-2 mt-3">
        <ProfileAvatarStack people={friendsPosted.map((f) => ({ name: f.display_name, src: f.avatar_url, username: f.username }))} size={24} />
      ...
      return (
        <Card>
          ...
          <Txt variant="subhead" tone="secondary" className="mt-3">
            Window closed. Say why, or it counts as a miss.
          </Txt>

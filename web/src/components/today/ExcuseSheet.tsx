...
export function ExcuseSheet({ occurrence: o, open, onOpenChange }: { occurrence: Occurrence; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  // Focus once the sheet has settled, without letting the browser scroll anything.
  useEffect(() => {
    if (!open) return;
    const t = window.setTimeout(() => fieldRef.current?.focus({ preventScroll: true }), 420);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange} title={`Can't make ${o.course_code}?`}>
      ...
      <TextArea value={reason} onChange={(e) => setReason(e.target.value.slice(0, EXPLANATION_MAX))} placeholder="dentist at 2, back for the lab" maxLength={EXPLANATION_MAX} ref={fieldRef} className="mt-3" />
